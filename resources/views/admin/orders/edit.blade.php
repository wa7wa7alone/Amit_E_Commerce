...
@section('content')
    ...
    <div class="content">
        ...
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">@lang('jobs.edit')</h1>
            <a class="btn btn-secondary float-end clearfix" href="{{ route('admin.jobs.index') }}">Back to
                @lang('jobs.plural')</a>
        </div>
        <form class="card row g-3 my-3" method="POST" action="{{ route('admin.jobs.update', $job->id) }}"
            ...
            <div class="card-body row">
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" id="description" cols="30" rows="10">{{ old('description', $job->description) }}</textarea>
                    <div class="invalid-feedback">
                ...
                <div class="col-12">
                    ...
                    <input type="number" class="form-control" id="views" name="views" placeholder=""
                        value="{{ old('views', $job->views) }}" required>
                    <div class="invalid-feedback">
                ...
                <div class="col-12">
                    ...
                    <select class="form-select" name="statu" id="statu" required>
                        <option @selected(old('statu', $job->statu) == 'pending') value="pending">@lang('jobs.status.pending')</option>
                        <option @selected(old('statu', $job->statu) == 'open') value="open">@lang('jobs.status.open')
                        </option>
                        <option @selected(old('statu', $job->statu) == 'closed') value="closed">@lang('jobs.status.closed')
                        </option>
                    </select>
                    <div class="invalid-feedback">
                ...
                <div class="col-12">
                    ...
                    <select class="form-select" name="category_id" id="category_id" required>
                        @foreach ($categories as $category)
                            <option @selected(old('category_id', $job->category_id) == $category->id) value="{{ $category->id }}">
                ...
                <div class="col-12">
                    ...
                    <select class="form-select" name="user_id" id="user_id" required>
                        @foreach ($users as $user)
                            <option @selected(old('user_id', $job->user_id) == $user->id) value="{{ $user->id }}">
                                {{ $user->name }}</option>
                        @endforeach
                    </select>
                    <div class="invalid-feedback">
